...
**我的初答**：
1. primary key主键索引列既不能重复也不能为null
2. unique只保证该列数据无重复,不保证不为null,底层索引保证不重复
3. foreign key保证数据不被随意修改,底层索引联合其他的表
4. 在InnoDB存储引擎中,主键索引会在叶子结点存储整行的数据,辅助索引只存放对应列的数据以及主键id
5. 自增主键长度小,占用的内存空间小,业务字段如身份证号太过冗长,且设计到用户的个人隐私和数据安全,不适合用来做主键,主键最好选择无具体业务关联的字段
6. UUID每次产生的数字都是随机的且数字较长,占用空间,用其做主键不像自增id会从左到右逐个添加进索引,而是会在各叶之间跳动,很可能会造成页分裂,降低性能

**错漏点**：
<details>
<summary><strong>点击展开错漏点</strong></summary>

1. PRIMARY KEY ⚠️ 特性对，底层实现没答

> 主键约束 = 唯一 + 非空，**底层通过聚簇索引实现**——约束的“不重复、不为空”检查就发生在向 B+ 树插入键值的过程中（键在有序树上天然不允许重复出现）。  
> **InnoDB 特有细节**：主键即聚簇索引，**每张表有且只有一个**；若不显式定义，InnoDB 会自动选用第一个 `NOT NULL UNIQUE` 索引，或生成隐藏的 ROW_ID（上一题已讲过，此处能复述是加分项）。

### 2. UNIQUE ⚠️ 前半对，“底层索引保证不重复”太笼统

需答出的底层细节：

> UNIQUE 约束底层是**一棵普通的二级索引 B+ 树（unique 索引）**。插入时沿树查找该键是否存在：存在 → 报 `Duplicate entry` 错误；不存在 → 插入。  
> **与主键的两个关键差异：**  
> ① **NULL 特殊待遇**：NULL 不参与唯一性判断，**可以插入多个 NULL**（你答对了这点，好）；  
> ② **插入性能差异（高频追问）**：
>
> - **普通索引**：查找到目标位置后**继续扫**到下一个不同值才能确认不重复？不——普通索引插入时不需要判重，找到插入位置直接插入，还要判断是否需要页分裂；
> - **唯一索引**：插入前必须先**判重**（读操作多一次）。
> - **change buffer**：普通索引插入可以利用 **change buffer** 把判重之外的开销延迟合并；唯一索引因为必须立刻读磁盘判重，**无法使用 change buffer**。因此在写多读少场景，普通索引的插入性能显著优于唯一索引——这也是为什么“业务代码已保证不重复时，唯一索引可以降级为普通索引”这个实践建议存在。

### 3. FOREIGN KEY ❌ 表述含糊，底层机制错误

❌ “底层索引联合其他的表”——这个说法不准确。

> 外键约束**不存在“跨表索引”**这种结构，它的实现是**逻辑检查机制**：  
> ① 子表的 FK 列上，MySQL 会**自动创建一个普通二级索引**（若已存在可复用）——用于加速父表校验时的查找；  
> ② 修改数据时的**联动检查/操作**由 InnoDB 在**执行层**完成，而非索引层：
>
> - 插入子表行 → 去父表对应索引上验证外键值存在（这就是为什么子表 FK 列要有索引）；
> - 删除/更新父表行 → 按 `RESTRICT`（禁止，默认）或 `CASCADE`（级联删除/更新子行）等策略处理。
>
> **加分项**：生产环境中**普遍不推荐使用外键**——每次写入都要跨表校验，高并发下带来锁竞争和性能损耗；通常在应用层保证引用完整性。阿里的 Java 开发规范明确禁用外键。这个观点说出来很体现工程素养。

### 4. 追问一：主键索引 vs 辅助索引的 B+ 树差异 ✅ 正确

补充一个关键词让答案更完整：

> 本质差异：**聚簇 vs 非聚簇**。主键索引叶子节点存**整行数据**（索引即数据，数据即索引，表本身就按主键序物理组织）；辅助索引叶子节点存**索引列值 + 主键值**，查完通常需要拿主键**回表**再走一次聚簇索引才能拿到整行。因此辅助索引查询路径 = 二级索引树 1~3 次 + 回表 1~3 次，天然比主键查询贵。  
> 若 SELECT 的列恰好都在二级索引里，则**覆盖索引**可免去回表。

### 5. 追问二：为什么用自增主键 ⚠️ 答了一半

你答了“长度小、省空间”和“业务解耦、隐私”——都有道理，但**丢掉了最重要的性能原因**：

> **① 顺序插入，避免页分裂（核心）**：  
> 自增值单调递增，新行永远追加到 B+ 树最右端，写满一页就顺序开新页——**磁盘顺序写**，页利用率 100%。  
> 业务字段（身份证号）值是**随机分布**的，插入位置随机落在树中间：目标页已满 → **页分裂**（申请新页 + 移动一半数据，且造成页内碎片），随机 I/O 陡增。
>
> **② 主键长度影响所有二级索引**：  
> 辅助索引叶子节点存的就是主键值，主键多长，**每个二级索引的每一行都跟着膨胀**。身份证 18 字节 vs BIGINT 自增 8 字节，一个有十几个索引的表差距可观。
>
> ③ 你提到的业务解耦、隐私合规（身份证做主键泄露风险、业务变更风险，如换号/回收）——属于次要但正确的理由。

### 6. 追问三：UUID 作主键的性能问题 ✅ 答得最好

你答到了“随机、长、页分裂”——方向全对。可以再补三个精确的点：

> ① **插入随机化**：UUID 完全无序（MySQL 8.0 之前的 v1/v4），新主键随机落在聚簇索引任意位置，**几乎每次插入都可能页分裂**，产生大量随机 I/O 和碎片；  
> ② **长度问题**：UUID 36 字符（标准字符串形式）/ 16 字节（binary），远大于 BIGINT 的 8 字节，**聚簇索引和所有二级索引同步膨胀**，内存缓冲池（buffer pool）能缓存的数据变少，命中率下降；  
> ③ **页利用率低**：随机插入导致页长期半满状态，同样数据量占用更多页，**树变高，查询 I/O 增加**。
>
> **加分项——解决方案**：
>
> - MySQL 8.0 提供 **UUID_SHORT()** / 或对 UUID v7（时间戳前缀，趋势递增）；
> - 分布式场景常用**雪花算法（Snowflake）**：64 位 ID，趋势单调递增，兼得全局唯一与顺序插入，是业务主键的主流选择。
</details>
...
**我的初答**：
1. 对于limit 10000,20 MySQL会逐行扫描数据前10020行数据,但只会保留10000后的20行数据.
- 第一种方案,适用于对每一页都要进行分页查询的时候,记住上一页最后一行的主键id,分页条件为当id>该主键id时分页,直接走主键索引
- 第二种方案: 通过覆盖索引+子查询的方式进行优化
2. 通过create_time的索引查询,再回表查询整行数据,可能存在多个订单在同一天进行,都使用这种可能会出现遗漏数据

**错漏点**：
<details>
<summary><strong>点击展开错漏点</strong></summary>

### 1. LIMIT 执行机制 ⚠️ 说对了“扫描后丢弃”，但丢了核心成本

你说“逐行扫描前 10020 行，只保留后面的 20 行”——流程对，但**没有解释慢在哪**。面试官要听的是这一句：

> `ORDER BY create_time DESC` 走的是 **create_time 二级索引**，但 `SELECT *` 需要整行数据。所以 MySQL 的实际动作是：  
> ① 沿 create_time 索引倒序扫描，定位到第 1 条；  
> ② **每扫到一条就回表一次**，取出整行；  
> ③ 扫满 10020 行、回表 **10020 次之后**，把前 10000 行全部丢弃，只返回 20 行。
>
> **慢的根源**：10000 次**随机 I/O 回表**全部做了无用功。丢弃的行越多，浪费越大——`LIMIT 1000000, 20` 就是百万次回表，越翻页越慢（“深分页”）。

丢掉这一点，“扫描后丢弃”只是现象描述，没有成本分析，等于没答为什么慢。

### 2. 方案一：游标分页 ⚠️ 思路对，但有概念混淆

❌ 你写的是“id > 上一页最后的主键 id，直接走主键索引”——**这和本题不匹配**。本题排序字段是 `create_time`，游标必须基于**排序字段本身**，否则结果的顺序和 WHERE id > x 的顺序对不上。正确写法是追问里的形式：

```
SELECT * FROM orders 
WHERE create_time < '上一页最后一条的 create_time'
ORDER BY create_time DESC 
LIMIT 20;
```

即：把 `OFFSET` 转化为 `WHERE` 条件，让索引直接**定位到第 10001 行的起点**，前面 1 万行完全不碰。这正是追问那条 SQL 的思路。

⚠️ 另外你没提这个方案的**适用限制**（这是标准答案的组成部分）：

- 只支持**连续翻页**（下一页/滚动加载），**不能跳页**——因为必须知道上一页的游标值；
- 中途有数据插入/删除时，结果集可能偏移（新插入的行被跳过或重复出现）。

### 3. 方案二：覆盖索引 + 子查询/延迟关联 ✅ 思路对，但太笼统

需要能写出具体 SQL（面试常要求现场写）：
...
- 子查询只 `SELECT id`——如果建立了 `(create_time, id)` 联合索引（或利用 InnoDB 二级索引叶子节点本身含主键的特性），子查询可以**在索引树上直接拿到 id，全程不回表**；
- 扫描 10020 行只发生在**窄索引**上（顺序 I/O，且每行只有十几字节）；
- 回表只发生在**最终 JOIN 的 20 次**上——回表次数从 10020 降到 20，这就是优化的本质。

性能量级：深分页场景下通常能快几十到上百倍。

### 4. 追问：游标分页的索引利用与隐患 ⚠️ 答了一半

**索引利用情况（你没答）：**

> `WHERE create_time < '2024-01-01' ORDER BY create_time DESC LIMIT 20` 完美利用 B+ 树的有序性：  
> ① range 定位到 `'2024-01-01'` 这个边界点；  
> ② 沿索引**倒序**扫描，天然有序，**无需 filesort**；  
> ③ 取 20 条即停，回表 20 次。  
> 无论翻到多深，代价恒定为 20 次回表——**这是游标分页相对于 OFFSET 的本质优势**。

**隐患（你答对了方向，但需展开）：**

你说“同一天多个订单可能遗漏数据”——正确，但更准确的表述是：

> **游标值重复导致翻页丢数据**。若上一页最后一条 `create_time = '2024-01-01 00:00:00'`，而**还有 3 条订单的时间戳与之完全相同**，下一次查询 `create_time < '2024-01-01 00:00:00'` 会把这 3 条**全部跳过**——它们永远无法出现在任何一页中。

**标准修复方案（必答）：复合游标**

用 `(create_time, id)` 做联合游标，条件改写为：
...
-- 上一页最后一条: (t0, id0)
SELECT * FROM orders
WHERE create_time < t0
   OR (create_time = t0 AND id < id0)
ORDER BY create_time DESC, id DESC
LIMIT 20;
```

id 主键保证唯一，作为第二排序键可以**彻底消除“同值翻页”的歧义**。若 `(create_time, id)` 上没有现成的联合索引，建议建一个，这样这个条件 + 排序可以完全走索引，仍然无需 filesort。

**其他可以顺带提的隐患（加分项）：**

- 游标分页**无法跳页**、无法支持“第 N 页”直达；
- 页间有新数据写入时，后续页内容会动态偏移（对滚动加载场景可接受，对分页报表场景不可接受）。
...
1. 数据库扩展性上存储过程难以扩展
2. 应用层不了解
3. 版本控制方面难以更新,CI/CD不了解
4. 首先考虑使用存储过程的统计之后维护是否可能出现问题,然后考虑存储过程是否复杂,相比与使用java层查询,带来的利是否大于弊

**错漏点**：
<details>
<summary><strong>点击展开错漏点</strong></summary>

### 1. 数据库扩展性（水平分库）——存储过程锁死了拆分的可能

- **微服务 + 分库分表是常态**：业务量上来后，单库扛不住，要按用户 ID、订单时间等做水平拆分（ShardingSphere / MyCat），一个逻辑表分散到几十个物理库。

- **存储过程绑架了"数据必须在同一个库"**：过程体内的 JOIN、子查询、临时表都默认数据在同一实例。一旦分库：

  - 过程里的跨表 JOIN 变成跨库 JOIN——**存储过程根本没有跨库能力**，全部要重写；

  - 重写的工作量在 DB 层无法复用 Java 生态的分片中间件，等于绕过了整个基础设施。

- **扩容成本不对称**：Java 应用无状态，加机器就能扩容；数据库加机器涉及数据迁移、路由规则、双写切换。逻辑写在存储过程里，意味着**每次扩容都要同时动 DB 逻辑**，扩容从"加机器"变成"做手术"。

- 一句话总结：**存储过程让数据库从"可替换的存储"变成"绑死业务的巨石"，失去了水平扩展的弹性。**

### 2. 应用层无状态设计——存储过程破坏了微服务的伸缩基石

- **无状态是自动扩缩容的前提**：Spring Boot 服务不保存会话、不绑定单机状态，K8s 可以随时拉起/杀掉任意实例，流量高峰时一键扩容。

- **逻辑下沉到 DB = 状态跟着逻辑走**：业务规则写在某个数据库实例里，这个库就成了"有状态的单点"——它不只是存数据，还"记着"业务规则。应用层看起来无状态，实际上所有服务都耦合在这一个 DB 实例的逻辑上。

- **连锁后果**：

  - **多服务复用同一存储过程 = 隐式共享耦合**：微服务要求服务间通过 API 通信，而多个服务调用同一个存储过程，等于绕开 API 契约直接共享了内部实现，改一个过程影响面无法评估；

  - **容量瓶颈上移**：逻辑在 Java 层，算力不够加应用实例（便宜）；逻辑在存储过程，算力不够只能加 DB 的 CPU/内存（贵且有上限）——**把最容易扩展的层级的压力，压到了最难扩展的层级上**。


### 3. 版本控制与 CI/CD——存储过程游离在工程体系之外
...
| 维度  | Java 代码 | 存储过程 |
| --- | --- | --- |
| **版本控制** | Git 管理，code review、blame、diff 完整 | 往往直接在生产库 `ALTER PROCEDURE`，代码飘在数据库里，Git 里的脚本和线上实际版本对不上 |
| **CR / 单测** | MR 评审 + JUnit 单测 | 基本没有评审环节，单测要搭真实库环境，成本高到没人写 |
| **发布** | CI 流水线一键构建发布 | 需要 DBA 手工执行脚本，无法进流水线，发布节奏被迫等 DBA |
| **回滚** | 镜像回滚，秒级 | 需要准备反向脚本手工执行，出错率高 |
| **环境一致性** | 同一份 jar 跑遍 dev/test/prod | dev 改了忘了同步 prod，环境间版本漂移，线上事故的经典来源 |

一句话总结：**Java 团队的所有工程能力（Git、CI/CD、单测、回滚）都是围绕"代码仓库"建的，存储过程不在这个体系内，成了运维盲区。**

## 三、追问：必须用存储过程做复杂报表时，如何权衡？

这是考你**不做极端决策、能给出工程化折中方案**的能力。答题框架分四步：

### 第一步：先质疑"必须用"这个前提（给出替代方案）

复杂报表效率低，通常有更工程化的解法，按优先级尝试：

1. **SQL 本身优化**：加索引、改写 SQL、用覆盖索引——80% 的"Java 层查询慢"其实是 SQL 没写好，不是非要用存储过程；

2. **预计算 / 离线数仓**：报表本质是 OLAP 场景（呼应上一题），把统计逻辑挪到离线任务（定时跑批写结果表、或同步到 ClickHouse/StarRocks 这类分析库），Java 层只查结果——**既快又不污染业务库**；

3. **Java 层做分步查询**：把一个大 SQL 拆成几个小查询在内存里聚合，牺牲一点开发复杂度换取可维护性。


### 第二步：确实绕不开时，给出"受控使用"的原则

如果评估后确认必须用（比如亿级数据实时聚合，离线方案不满足时效）：

- **限定边界**：存储过程**只做"数据密集型计算"，不含业务规则**——业务判断（什么状态算有效订单、费率怎么算）仍留在 Java 层。这样存储过程退化为"一个跑在 DB 里的重查询"，而不是"业务逻辑";

- **纳入工程体系**：过程脚本必须进 Git 仓库、走 code review、由 CI 流水线自动发布（如用 Flyway/Liquibase 管理 DB 变更），回滚脚本同步准备——**把存储过程当代码管理，而不是当数据库对象管理**;

- **文档与归属**：明确 owner，注释写清楚"为什么这里必须用存储过程"，防止后人无脑效仿。


### 第三步：一句话总结决策标准

> **"性能收益是确定的，维护成本是长期的。"** 只有当性能差距达到数量级（如秒级 vs 分钟级）、且无法通过索引/离线数仓解决时，才接受存储过程；一旦采用，就通过"只算不判 + 进 Git + 走流水线"把它的运维成本压到最低。

---

## 四、面试答题技巧

你这次的"不了解"集中在**应用层和 CI/CD**，说明对微服务的工程体系还不熟。建议补三块知识：

1. **无状态服务**：为什么无状态才能自动扩缩容（K8s HPA 的原理）；

2. **CI/CD 流水线**：一次 Java 发布从 commit 到上线经历哪些环节，存储过程在哪个环节掉队；

3. **Flyway / Liquibase**：数据库变更怎么像代码一样做版本管理——这是追问里"受控使用"的落地工具，答出来是明显加分项。
...
## Day 47 (2026-09-10) —— 触发器与锁（全局锁、表锁）的 Java 后端应用

### 题目1：触发器在 Spring Boot 事务中的“隐蔽陷阱”（业务逻辑与触发器耦合）
> 在 Java 后端开发中，有时会使用触发器（Trigger）自动更新日志表或统计数据。请回答以下问题：
> 1. 在 Spring 的 `@Transactional` 方法中执行 `INSERT INTO orders`，若该表上存在一个 `AFTER INSERT` 触发器（用于更新统计表），当触发器执行失败（如违反唯一约束）时，Spring 事务会回滚吗？外层 Java 代码捕获的异常是什么？
> 2. 如果触发器执行了 `UPDATE` 操作，而该操作又触发了另一个表上的触发器，导致触发器链无限循环，MySQL 如何检测并处理？Java 应用层会收到什么错误？
> 3. 为什么阿里开发规范通常**禁止使用触发器**？在微服务架构中，触发器的存在会给分布式事务（如 Seata）带来什么问题？

<details>
<summary><strong>点击展开标准解析</strong></summary>

- 问题1：触发器失败会导致整个 SQL 语句失败，进而导致 Spring 事务回滚（因为底层 JDBC 抛出 `SQLException`）。Spring 会将 `SQLException` 转换为 `DataIntegrityViolationException` 或 `UndeclaredThrowableException`。但触发器错误信息（如自定义 `SIGNAL`）会丢失在异常链中，难以定位具体业务原因。Java 层无法感知触发器内部的业务语义，只能通过解析 SQLState 间接处理。
- 问题2：MySQL 通过 `max_sp_recursion_depth`（默认 0）控制触发器递归。若触发器自身触发自身（或形成循环），MySQL 会直接报错 `Recursive limit 0 (as set by the max_sp_recursion_depth variable)`，导致整个事务回滚。Java 层会收到 `SQLException`，但错误信息可能不明确，导致排查困难。
- 问题3（禁用原因）：
  1. **事务边界模糊**：微服务中，若一个服务通过触发器修改了另一个服务的表（或日志），本质上是“隐式分布式事务”，Seata 无法感知触发器的存在，导致分布式事务失效。
  2. **可维护性差**：触发器逻辑隐藏在数据库层，脱离 Java 代码的版本控制（Git），且难以进行单元测试。排查问题时，DBA 需与开发联合调试，效率极低。
  3. **性能不可控**：触发器会隐式增加主 SQL 的执行开销（尤其当触发器包含复杂 `SELECT` 时），且难以通过 `EXPLAIN` 单独分析。
</details>

**我的初答**：
1. Spring还未学到,但触发器执行失败可能会导致Spring事务回滚,外层java捕获的异常不了解
2. 不了解如何处理
3. 因为触发器存在隐患,比如因为是隐式触发,不手动查询就无法知道是否有触发器,出现问题难以排查,再者就是存在隐式性能消耗,查100万行数据,触发100万次select触发器,然后就是题目中提到的无限循环问题
4. 微服务架构没学

**错漏点**：


### 题目2：全局锁（FLUSH TABLES WITH READ LOCK）与表锁（LOCK TABLES）在 Java 应用中的风险
> 在 Java 后端执行备份或批量 ETL 时，DBA 可能使用全局锁或表锁。请回答：
> 1. 若 Spring Boot 应用在 DBA 执行 `FLUSH TABLES WITH READ LOCK`（全局只读锁）期间，尝试执行 `INSERT` 或 `UPDATE` 操作，会发生什么？该操作会超时吗？超时时间由哪个参数控制？
> 2. 在 MyBatis 中，若使用 `SELECT ... FOR UPDATE` 或 `LOCK TABLES orders WRITE`，锁释放前若应用服务重启（如 `kill -9`），该锁会被释放吗？为什么？
> 3. 在高并发场景下，使用 `LOCK TABLES` 表锁与 InnoDB 行锁相比，对 HikariCP 连接池的排队效应有何不同？为什么？

<details>
<summary><strong>点击展开标准解析</strong></summary>

- 问题1：应用层的 `INSERT`/`UPDATE` 会被阻塞，直到全局锁释放（`UNLOCK TABLES`）。具体行为：
  - 默认情况下，SQL 会一直等待（无限期），直到锁释放或事务超时（`lock_wait_timeout` 参数，默认 31536000 秒，即 1 年，对于全局锁，实际受 `wait_timeout` 影响）。
  - 若连接超过 `wait_timeout`（默认 8 小时），连接会被断开，导致应用抛出 `CommunicationsException`。
- 问题2：**会被释放**。因为锁是绑定到 `Connection`（或会话）的。应用服务重启会导致连接断开（`kill -9` 会强制终止 JVM，底层 Socket 关闭），MySQL 服务端检测到连接断开后，自动释放该会话持有的所有锁。
  - 风险：若锁未及时释放（如网络中断但服务端未感知），可通过 `SHOW PROCESSLIST` 手动杀死会话（`KILL` 命令）。
- 问题3：表锁是粗粒度锁，一旦执行 `LOCK TABLES orders WRITE`，会导致该表的**所有读写请求串行化**，HikariCP 中的大量连接会因获取不到锁而阻塞，连接池被快速耗尽（`Connection is not available`）。而行锁（InnoDB）仅锁定特定行，其他行的读写不受影响，连接池可正常复用。因此高并发系统必须使用 InnoDB 行锁，**禁止手动加表锁**。
</details>
...
### 题目3：线上死锁的 Java 层排查与重试策略（结合行锁与表锁）
> 生产环境出现 `Deadlock found when trying to get lock; try restarting transaction` 错误。作为 Java 后端开发，请回答：
> 1. 死锁日志（`SHOW ENGINE INNODB STATUS`）中的 `LATEST DETECTED DEADLOCK` 部分，哪些关键信息能帮助定位是哪两个 SQL 语句造成了死锁？
> 2. 在 Spring 中，通常推荐的应对策略是**自动重试机制**（Retry）。请写出使用 `@Retryable`（Spring Retry）实现死锁重试的核心代码，并说明重试时需注意什么（如幂等性、最大重试次数）。
> 3. 若死锁频繁发生，且涉及 `LOCK TABLES` 或全局锁，为何重试策略通常无效？此时应优先考虑什么架构优化？

<details>
<summary><strong>点击展开标准解析</strong></summary>

- 问题1（定位死锁）：
  在 `LATEST DETECTED DEADLOCK` 中，重点关注：
  - `(1) TRANSACTION` 和 `(2) TRANSACTION` 部分，查看 `WAITING FOR THIS LOCK TO BE GRANTED` 和 `HOLDS THE LOCK(S)`，可定位是哪些行或间隙发生了冲突。
  - 每个事务下的 SQL 语句（`MySQL thread id ...`），可直接复制出对应的 `UPDATE` 或 `SELECT ... FOR UPDATE` 语句。
  - 通常，死锁发生在两个事务以相反的顺序更新同一张表（如事务1更新 A->B，事务2更新 B->A）。
- 问题2（重试代码与注意事项）：
  - 配置与示例：
    ```java
    @Retryable(
    ...
    ```
  - 注意点：
    1. **幂等性**：重试执行的逻辑必须支持幂等（可使用乐观锁版本号，或基于业务唯一键判断），避免重复扣减库存或积分。
    2. **最大重试次数**：不宜超过 3~5 次，否则加剧数据库负载。
    3. **异常匹配**：Spring 会将 `SQLException` 转换为 `DeadlockLoserDataAccessException`，需正确配置。
- 问题3（表锁死锁处理）：
  - `LOCK TABLES` 导致的死锁通常是**全局性阻塞**（所有事务排队），而非 InnoDB 行级冲突，重试只会让大量线程反复竞争，无法解决根本问题。
  - 架构优化方向：
    1. **升级为 InnoDB 行锁**：若使用 MyISAM，需迁移至 InnoDB。
    2. **优化事务粒度**：将长事务拆分为多个小事务，减少锁持有时间。
    3. **引入分布式锁（如 Redis）**：在 Java 层控制并发，避免 MySQL 层锁竞争（如扣减库存时使用 Redis 预扣）。
</details>
...
## Day 48 (2026-09-11) —— 行级锁与 InnoDB 引擎结构（Java 后端面试）

### 题目1：InnoDB 行级锁的三种形式（Record Lock、Gap Lock、Next-Key Lock）及加锁规则
> 现有表 `orders`，主键 `id`，普通索引 `idx_user_id` 在 `user_id` 上。请分析以下 SQL 在 `REPEATABLE READ` 隔离级别下分别加了什么锁：
> 1. `SELECT * FROM orders WHERE id = 10 FOR UPDATE;`（id 为主键）
> 2. `SELECT * FROM orders WHERE user_id = 100 FOR UPDATE;`（user_id 有普通索引）
> 3. `SELECT * FROM orders WHERE user_id > 100 FOR UPDATE;`（范围查询）
> 4. `SELECT * FROM orders WHERE user_id = 100;`（普通 SELECT，无 FOR UPDATE）
     > 追问：为什么在 `READ COMMITTED` 隔离级别下，上述加锁行为会有所不同？Java 后端开发中，如何避免因间隙锁导致的并发插入阻塞？

<details>
<summary><strong>点击展开标准解析</strong></summary>

- 加锁分析（RR 隔离级别）：
  1. `WHERE id = 10 FOR UPDATE`（主键等值）：加 **Record Lock**（行锁），仅锁定 id=10 这一行。因为主键是唯一索引，等值查询能精确定位，无需加间隙锁。
  2. `WHERE user_id = 100 FOR UPDATE`（普通索引等值）：加 **Next-Key Lock**（Record Lock + Gap Lock），锁定 user_id=100 的记录以及其前后的间隙。因为普通索引可能有多条记录，为防止幻读，需要锁住间隙防止其他事务插入 user_id=100 的新行。
  3. `WHERE user_id > 100 FOR UPDATE`（范围查询）：加 **Next-Key Lock**，锁定所有满足条件的记录及其间隙（直到不满足条件的第一个值）。例如 user_id 有 100, 200, 300，则锁定 (100, 200] 和 (200, 300] 等区间，以及 300 之后的间隙。
  4. `WHERE user_id = 100`（普通 SELECT）：**快照读，不加锁**。在 RR 下读取 MVCC 快照（但用户还没学 MVCC，可简单说“一致性读，不加锁”）。
- RC 隔离级别差异：
  - RC 下，普通索引等值查询仅加 **Record Lock**，不加 Gap Lock（因此可能出现幻读）。
  - 范围查询仍会加间隙锁，但 RC 会释放不满足条件的间隙锁，锁范围更小。
- Java 后端避免间隙锁阻塞：
  1. 尽量使用主键或唯一索引进行等值查询（避免 Next-Key Lock）。
  2. 若必须使用普通索引，考虑在 RC 隔离级别下运行（但需业务容忍幻读）。
  3. 控制事务长度，尽快提交，减少锁持有时间。
  4. 高并发插入场景，可评估将隔离级别降为 RC（如互联网业务常用 RC）。
</details>
...
3. 加范围临键锁
4. 不加锁
5. 在READ COMMITTED隔离级别下不支持Gap Lock和Next-Key Lock,关于第3个只会给每行加上Record Lock
6. 改到RC隔离级别,或者对于普通查询不加for update,只对可能出现幻读的查询加锁

**错漏点**：
<details>
<summary><strong>点击展开错漏点</strong></summary>

### 第 1 条：`WHERE id = 10 FOR UPDATE`（主键）✅ 结论对，缺一个分支

> 主键是**唯一索引**，等值命中唯一记录后，InnoDB 确认只有这一行，**Next-Key Lock 退化为 Record Lock**——只锁 id=10 这一行（锁在聚簇索引上）。别的行随便插、随便改。
>
> ❌ **你漏掉的分支**：若 id=10 的记录**不存在**，InnoDB 会加 **Gap Lock**（锁住 10 应该所在的那个间隙），防止其他事务插入 id=10 的行。“命中退化为记录锁 / 不命中加间隙锁”是这个考点的完整答案，只答一半丢分。

### 第 2 条：`WHERE user_id = 100 FOR UPDATE`（普通索引）❌ 错了，这是本题核心

**不能只加 Record Lock。** 因为 user_id 是**非唯一**索引：可能有 3 行 user_id=100，如果只锁这 3 行，别的事务随时可以**再插入一行 user_id=100**（幻读）。RR 级别必须防幻读，所以：

> 假设索引中 user_id 的值有 `... 90, 100, 100, 100, 110 ...`，加锁为：  
> ① 对**所有** user_id=100 的索引记录加 **Next-Key Lock**：`(90, 100]`、`(100, 100]`、`(100, 100]`；  
> ② 对最后一个 100 之后的间隙加 **Gap Lock**：`(100, 110)`——防止在这个间隙里再插入一个 user_id=100；  
> ③ 每一条命中的行，还要在**聚簇索引（主键）上加 Record Lock**。
>
> **后果**：不仅插入 user_id=100 会被阻塞，连插入 user_id=105（落间隙内）也会被误伤阻塞——这正是间隙锁“锁范围扩大”的代价。

**对比记忆（必背）**：

| 查询条件 | 唯一索引 | 普通索引 |
| --- | --- | --- |
| 等值命中 | Record Lock（退化） | 所有匹配记录 Next-Key Lock + 末尾 Gap Lock + 主键记录锁 |
| 等值未命中 | Gap Lock | Gap Lock |
| 范围  | 范围内 Next-Key Lock | 范围内 Next-Key Lock |

### 第 3 条：`WHERE user_id > 100 FOR UPDATE` ✅ 结论对

> 对**索引中所有 user_id > 100 的记录**（含边界后第一个区间 `(100, 首个>100的值]`）加 **Next-Key Lock**，一直锁到索引末尾的** supremum 伪记录（+∞）**；命中行同时在主键上加 Record Lock。
> 效果：任何 `user_id > 100` 的插入全部阻塞。
> ⚠️ 可补充的经典陷阱：若 WHERE 条件的列**没有索引**，InnoDB 只能全表扫描，**给所有记录都加 Next-Key Lock，效果等于锁全表**——生产事故高发点。

### 第 4 条：普通 SELECT ✅ 正确，但要说原因

> 不加任何锁。因为普通 SELECT 走的是 **MVCC 快照读**：RR 下事务内第一次查询建立 ReadView，之后读的都是这个快照，读操作不加锁、不阻塞写。加锁的只有 `FOR UPDATE` / `LOCK IN SHARE MODE` / `UPDATE` / `DELETE` 这类**当前读**。  
> “不加锁”三个字后面跟上“因为快照读”才完整。

## 二、追问一：RC 下加锁行为为什么不同？⚠️ 结论对，“为什么”没答

你说“RC 不支持 Gap Lock / Next-Key Lock”——方向对但两处不严谨：

> **① 严谨的表述**：RC 下间隙锁**基本禁用**（并非完全没有——外键检查、唯一键冲突检测这两个场景仍会用到），加锁退化为**只对命中的行加 Record Lock**，且扫描到不满足条件的行时锁会**即时释放**（RR 会持有到事务结束）。
>
> **② 为什么（题目问的正是这个，你没答）**：  
> 间隙锁**唯一的存在目的就是防幻读**。RR 要求“事务内多次读取结果一致”，所以必须用间隙锁封住“可能插入新行的空隙”；而 RC 的设计本来就**允许幻读**——每条语句都生成新的 ReadView，能看到别人新提交的插入。既然允许幻读，防幻读的间隙锁就没有存在的必要，砍掉它换来更高的并发。
>
> **③ 因果关系要理顺**：不是“RC 性能差所以不支持间隙锁”，而是“RC 不承诺防幻读 → 不需要间隙锁 → 并发更高”。加锁行为是隔离级别语义的结果，不是原因。

### 追问二：Java 后端如何避免间隙锁阻塞插入？⚠️ 方向对，建议不完整

你答的“改到 RC”是主答案之一，但后半句有问题：**“普通查询不加 FOR UPDATE”——普通 SELECT 本来就不加锁**（第 4 条已说明），这个建议暴露了概念混淆。“只对可能出现幻读的查询加锁”也过于含糊。完整答案：

> **① 隔离级别**：把不需要 RR 语义的业务切到 RC（互联网公司主流做法，RR 换 RC，用乐观机制兜底一致性）；
>
> **② 让查询退化为记录锁**：能给列建**唯一索引**的就建，等值查询命中唯一索引只加记录锁，天然无间隙锁——从设计上消灭而不是规避；
>
> **③ 悲观锁换乐观锁**：用 `version` 字段 + CAS 式 UPDATE 代替 `SELECT FOR UPDATE`，写冲突时重试，不长时间持锁；
>
> **④ 缩短事务持锁时间（Java 特有的坑）**：`@Transactional` 方法里**不要调 RPC / HTTP / 发消息**等慢操作——锁在事务提交前一直持有，慢调用期间间隙锁把别人的插入全堵死。把“锁计算”和“慢 IO”拆开是后端最实用的手段；
>
> **⑤ 理解底层机制（加分项）**：INSERT 前要先拿**插入意向锁**，它与别的事务的 Gap Lock 冲突，所以插入被阻塞。更隐蔽的是**死锁经典场景**：Gap Lock 之间互相兼容——两个事务可以同时持有同一个间隙的 Gap Lock，然后都往这个间隙 INSERT，各自被对方的 Gap Lock 挡住 → **互相等待成死锁**。规避方式：捕获 `Deadlock found when trying to get lock` 异常并重试，同时保证多事务按**固定顺序**访问/插入数据。

</details>

### 题目2：InnoDB 内存结构（Buffer Pool、Change Buffer、Log Buffer）及其对 Java 应用性能的影响
> 请回答以下关于 InnoDB 内存结构的问题：
> 1. `Buffer Pool` 的作用是什么？它如何减少磁盘 I/O？Java 后端开发中，为什么说“全表扫描”会严重污染 Buffer Pool，进而拖慢其他查询？
> 2. `Change Buffer` 是什么？它针对什么类型的操作进行优化？为什么唯一索引（UNIQUE）不能使用 Change Buffer？
> 3. `Log Buffer` 与 `redo log` 的关系是什么？Java 应用通过 JDBC 执行 `commit` 时，`innodb_flush_log_at_trx_commit` 参数为 1 和 2 分别对性能和数据安全性有何影响？

<details>
<summary><strong>点击展开标准解析</strong></summary>

- Buffer Pool：
  - 作用：InnoDB 在内存中缓存数据页和索引页，读写操作首先在 Buffer Pool 中进行，修改后的脏页异步刷盘。极大减少磁盘随机 I/O。
  - 全表扫描的危害：全表扫描会将大量不常访问的页加载到 Buffer Pool，挤占热点数据（如高频访问的用户信息），导致缓存命中率下降，其他查询被迫从磁盘读取，整体性能骤降。
- Change Buffer：
  - 作用：针对**非唯一普通索引**的写操作（INSERT、UPDATE、DELETE），若索引页不在 Buffer Pool 中，InnoDB 不会立即从磁盘读取索引页，而是将变更缓存在 Change Buffer 中，待后续读取该页时再合并。减少随机磁盘读。
  - 唯一索引不能使用：因为唯一索引需要校验唯一性，必须将索引页读入内存判断是否冲突，无法延迟合并，因此 Change Buffer 不适用。
- Log Buffer 与 redo log：
  - Log Buffer 是 redo log 的内存缓冲区。事务提交时，redo log 先写入 Log Buffer，再根据 `innodb_flush_log_at_trx_commit` 参数决定何时刷盘。
  - 参数 1（默认）：每次事务提交都刷盘（fsync），保证数据不丢失，但性能最低（磁盘 I/O 频繁）。
  - 参数 2：每次提交写入 OS 缓存，每秒刷盘一次。若 MySQL 进程崩溃，数据不丢；若操作系统崩溃，可能丢失 1 秒数据。性能优于 1。
  - 参数 0：每秒刷盘一次，提交不触发刷盘。性能最高，但 MySQL 崩溃会丢失最多 1 秒事务。
- Java 实践：金融级业务必须设为 1；高并发互联网业务可设为 2，兼顾性能与安全。
</details>
...
### 题目3：Java 应用中的行锁死锁排查与预防（结合 Spring 事务与 JDBC）
> 在 Spring Boot 项目中，两个 `@Transactional` 方法因更新同一张表的顺序不同，导致 MySQL 报 `Deadlock found when trying to get lock`。请回答：
> 1. 如何通过 MySQL 的 `SHOW ENGINE INNODB STATUS` 找到死锁涉及的两条 SQL 和加锁顺序？
> 2. 在 Java 代码层面，有哪些策略可以预防行锁死锁？（至少说出三种，如按固定顺序更新、减小事务粒度、使用乐观锁等）
> 3. 若死锁已发生，使用 Spring Retry 的 `@Retryable` 进行重试时，为什么必须保证业务逻辑的幂等性？请举例说明若不幂等会导致什么后果。

<details>
<summary><strong>点击展开标准解析</strong></summary>

- 死锁排查：
  - 执行 `SHOW ENGINE INNODB STATUS\G`，在 `LATEST DETECTED DEADLOCK` 部分，会显示两个事务分别持有的锁和等待的锁，以及具体执行的 SQL 语句。根据 SQL 可定位到 Java 代码中的更新顺序。
- 预防策略：
  1. **固定更新顺序**：所有事务按相同顺序更新表（如先更新 A 表再更新 B 表），避免交叉等待。
  2. **减小事务粒度**：将大事务拆分为小事务，减少锁持有时间，降低冲突概率。
  3. **使用乐观锁**：通过版本号（version）或时间戳，在更新时检查，避免长时间持有行锁。
  4. **降低隔离级别**：在业务允许的情况下使用 READ COMMITTED，减少间隙锁。
  5. **索引优化**：确保 WHERE 条件使用索引，避免全表扫描导致的锁范围扩大。
- 重试与幂等：
  - `@Retryable` 捕获死锁异常后自动重试，但若业务逻辑不幂等（如重复扣减库存），重试会导致数据错误。
  - 示例：扣减库存操作若未做幂等（如未记录已扣减的唯一业务 ID），第一次执行成功但返回超时，重试会再次扣减，导致超卖。
  - 解决方案：使用唯一业务键（如订单号）或乐观锁版本号，确保多次执行结果一致。
</details>
...
## Day 49 (2026-09-12) —— MVCC 多版本并发控制（Java 后端面试）

### 题目1：MVCC 的底层实现原理（隐藏字段、undo log 版本链、ReadView）
> 请解释 InnoDB 的 MVCC 是如何实现“可重复读”和“读已提交”的？请从以下三个核心组件说明：
> 1. 每行记录的隐藏字段（DB_TRX_ID、DB_ROLL_PTR）分别存储什么？
> 2. undo log 版本链是如何串联多个历史版本的？
> 3. ReadView 中的核心属性（m_ids、min_trx_id、max_trx_id、creator_trx_id）分别代表什么？RC 和 RR 隔离级别下，ReadView 的生成时机有何本质区别？
     > 追问：为什么 RR 级别下，同一个事务多次普通 SELECT 能读到相同的数据？
...
- undo log 版本链：
  - 每次修改一行，都会在 undo log 中记录旧值，并通过 DB_ROLL_PTR 将当前行与旧版本串联成链表。链表头部是最新版本，尾部是最旧版本。
- ReadView 核心属性：
  - m_ids：生成 ReadView 时活跃（未提交）的事务 ID 列表。
  - min_trx_id：m_ids 中的最小值。
  - max_trx_id：生成 ReadView 时下一个将要分配的事务 ID（即当前最大事务 ID + 1）。
  - creator_trx_id：创建该 ReadView 的事务 ID。
- 可见性判断规则：
  1. 若 DB_TRX_ID == creator_trx_id，说明是本事务修改的，可见。
  2. 若 DB_TRX_ID < min_trx_id，说明该版本在 ReadView 生成前已提交，可见。
  3. 若 DB_TRX_ID >= max_trx_id，说明该版本在 ReadView 生成后才开启，不可见。
  4. 若 min_trx_id <= DB_TRX_ID < max_trx_id，则判断 DB_TRX_ID 是否在 m_ids 中：若在，说明是活跃事务（未提交），不可见；若不在，说明已提交，可见。
  - 不可见时，沿 DB_ROLL_PTR 找到上一个版本，重复判断，直到找到可见版本或链表结束。
- RC 与 RR 的本质区别：
  - RC：每次执行普通 SELECT 都会生成一个新的 ReadView，因此能读到其他事务已提交的最新数据。
  - RR：只在事务第一次执行普通 SELECT 时生成 ReadView，后续复用，因此整个事务期间看到的是同一个快照，实现可重复读。
- 追问答案：RR 下首次 SELECT 生成 ReadView 后，后续其他事务的提交不会影响该 ReadView 的判断，因此多次读取结果一致。
</details>
...
### 题目2：快照读与当前读的区别，以及 MVCC 与锁的配合
> 在 InnoDB 中，普通的 SELECT 属于快照读，而 SELECT ... FOR UPDATE / UPDATE / DELETE 属于当前读。请回答：
> 1. 快照读和当前读在读取数据时分别使用什么机制？当前读为什么需要加锁？
> 2. 在 REPEATABLE READ 下，同一个事务中先执行普通 SELECT，再执行 SELECT ... FOR UPDATE，两者读到的数据可能不一致，为什么？这是设计缺陷吗？
> 3. Java 后端开发中，什么业务场景必须使用当前读？请举两个例子，并说明如果误用快照读会导致什么业务问题。
     > 追问：MVCC 能否完全替代锁？为什么？
...
- 快照读 vs 当前读：
  - 快照读：普通 SELECT，基于 MVCC 读取 ReadView 对应的历史版本，不加锁。
  - 当前读：读取最新已提交版本，并加锁（共享锁或排他锁），用于保证数据修改的安全性。
- RR 下两者不一致的原因：
  - 快照读使用事务首次 SELECT 时生成的 ReadView，读的是历史快照。
  - 当前读必须读取最新数据（否则更新会基于旧值，导致丢失更新），因此会绕过 MVCC 快照，读取最新版本并加锁。
  - 这不是缺陷，而是有意设计：快照读追求高并发读性能，当前读追求数据修改的准确性和并发控制。
- 必须使用当前读的场景：
  1. 库存扣减：先 SELECT ... FOR UPDATE 锁定库存行，再更新，防止超卖。若用快照读，两个事务可能读到相同库存，导致超卖。
  2. 账户余额更新：同样需要锁定账户行，避免并发更新导致余额错误。
- MVCC 不能完全替代锁：
  - MVCC 只解决读-写冲突（读不阻塞写，写不阻塞读），但无法解决写-写冲突。
  - 两个事务同时修改同一行时，必须通过锁来串行化，否则会丢失更新。
  - 因此 MVCC 与锁是互补关系，共同保证隔离性和并发性能。
</details>
...
### 题目3：MVCC 与长事务的隐患（undo log 膨胀、版本链过长）及 Java 后端控制策略
> 在 Java 后端开发中，若一个 Spring 事务长时间未提交（如包含远程调用、文件 IO、复杂计算），会对 InnoDB 的 MVCC 造成什么影响？请回答：
> 1. 长事务为什么会导致 undo log 无法及时清理？版本链过长会带来什么性能问题？
> 2. 如何通过 MySQL 命令排查当前是否存在长事务？（至少说出两种方式）
> 3. 在 Spring 中，如何有效控制事务长度？请给出至少三种实践建议。
     > 追问：若业务确实需要长时间处理，应如何拆分事务以保证数据一致性？

<details>
<summary><strong>点击展开标准解析</strong></summary>

- 长事务对 MVCC 的影响：
  - 长事务持有的 ReadView 会一直存在，导致其可见的旧版本不能被 purge 线程清理（因为可能有其他事务需要读取这些旧版本）。
  - undo log 版本链不断增长，占用大量磁盘空间，同时每次读取都要沿版本链回溯，增加 CPU 和 I/O 开销，严重拖慢查询。
- 排查长事务：
  1. 查询 information_schema.innodb_trx 表：SELECT * FROM information_schema.innodb_trx WHERE TIME_TO_SEC(TIMEDIFF(NOW(), trx_started)) > 60; 找出运行超过 60 秒的事务。
  2. 使用 SHOW ENGINE INNODB STATUS，查看 TRANSACTIONS 部分，关注 ACTIVE 时间较长的事务。
  3. 使用 performance_schema.events_transactions_current 表（MySQL 5.7+）。
- Spring 中控制事务长度：
  1. 避免在 @Transactional 方法中进行远程调用、HTTP 请求、文件读写等耗时操作，将这些操作移到事务外。
  2. 减小事务粒度：将一个大事务拆分为多个小事务，每个小事务快速提交。
  3. 设置事务超时：@Transactional(timeout = 5) 指定超时秒数，超时自动回滚。
  4. 使用编程式事务（TransactionTemplate）精确控制事务边界，避免声明式事务范围过大。
- 长时间处理的拆分策略：
  - 采用“异步 + 状态机”：先快速提交核心业务（如创建订单），然后通过消息队列异步处理后续耗时逻辑（如发送通知、生成报表），每个步骤独立事务。
  - 若必须保证一致性，可使用 TCC 或 Saga 模式，将长事务拆分为多个可补偿的短事务。
</details>
...
## Day 50 (2026-09-13) —— 日志、主从复制、分库分表、读写分离（Java 后端面试）

### 题目1：redo log、undo log、binlog 的区别及两阶段提交，主从复制中 binlog 的作用
> 请从作用、所属层次、写入时机、日志格式四个维度对比 redo log、undo log、binlog。解释 MySQL 如何通过两阶段提交保证 redo log 与 binlog 的一致性。在主从复制中，binlog 扮演什么角色？主从延迟的常见原因有哪些？
> 追问：若数据库崩溃后重启，InnoDB 如何利用 redo log 和 undo log 恢复数据？

<details>
<summary><strong>点击展开标准解析</strong></summary>

- 三者对比：
  - redo log：InnoDB 引擎层物理日志，记录数据页的物理修改（如“页 X 偏移 Y 改为 Z”），循环写，用于崩溃恢复，保证持久性。写入时机：事务执行过程中不断写入，提交时刷盘（由 innodb_flush_log_at_trx_commit 控制）。
  - undo log：InnoDB 引擎层逻辑日志，记录数据修改前的旧值，用于事务回滚和 MVCC 快照读。写入时机：事务修改数据前写入。
  - binlog：MySQL Server 层逻辑日志，记录所有修改数据的 SQL 语句或行变更，追加写，用于主从复制和数据恢复。写入时机：事务提交前写入（两阶段提交的 prepare 阶段写入 redo log，commit 阶段写入 binlog）。

- 两阶段提交：
  1. 事务提交时，InnoDB 先写 redo log，标记为 prepare 状态。
  2. 然后 Server 层写 binlog。
  3. 最后 InnoDB 将 redo log 标记为 commit 状态。
     若崩溃发生在步骤 2 之前，重启后 redo log 为 prepare 但无 binlog，则回滚事务；若 binlog 已写入，则提交事务。保证 redo log 和 binlog 逻辑一致。

- 主从复制中 binlog 的作用：
  - 主库将 binlog 发送给从库，从库通过 I/O 线程写入 relay log，再由 SQL 线程重放，实现数据同步。
  - 主从延迟原因：从库单线程重放（MySQL 5.6 前）、主库并发写入高、从库硬件差、大事务、网络延迟等。

- 追问：崩溃恢复时，InnoDB 先检查 redo log，将已提交（commit）的事务重做（前滚），对未提交（prepare 且无 binlog）的事务利用 undo log 回滚（后滚）。
</details>
...
### 题目2：分库分表的时机、拆分方式与核心难题（分片键、跨库 JOIN、全局 ID、分布式事务）
> 现有订单表 orders 数据量达到 2 亿行，查询缓慢。请回答：
> 1. 什么时候需要考虑分库分表？垂直拆分和水平拆分的区别是什么？
> 2. 水平分片键应如何选择？若选择 user_id 作为分片键，会带来什么问题？选择 order_id 呢？
> 3. 分库分表后，如何解决跨库 JOIN、分页排序、全局唯一 ID、分布式事务这四大难题？
     > 追问：使用 ShardingSphere 时，分片算法有哪些？如何避免数据倾斜？

<details>
<summary><strong>点击展开标准解析</strong></summary>

- 分库分表时机：单表数据量超过 500 万~1000 万行，或单库磁盘/连接数达到瓶颈时，考虑分库分表。垂直拆分：按业务模块拆分不同库（如订单库、用户库），或按字段冷热拆分（将不常用大字段拆到扩展表）。水平拆分：将同一表的数据按规则分散到多个库/表。

- 分片键选择：
  - 选择 user_id：能保证同一用户的数据落在同一分片，便于查询用户订单列表，但若按订单号查询则需扫描所有分片。
  - 选择 order_id：能保证订单均匀分布，但查询某用户的所有订单需跨多个分片。
  - 常用方案：采用基因法，将 user_id 后几位融入 order_id，兼顾两种查询；或使用双写冗余表。

- 四大难题解决：
  1. 跨库 JOIN：避免 JOIN，通过冗余字段或多次查询在应用层组装；或使用宽表。
  2. 分页排序：若按分片键排序，各分片取前 N 条再归并；若跨分片，需在内存中归并（性能差），建议限制深度分页。
  3. 全局唯一 ID：使用雪花算法、号段模式（如美团 Leaf）、Redis 自增等。
  4. 分布式事务：使用 Seata AT/TCC、消息最终一致性、本地消息表等。

- 追问：ShardingSphere 分片算法包括取模、哈希、范围、日期等。避免数据倾斜需选择分布均匀的字段作为分片键，或使用一致性哈希。
</details>
...
### 题目3：读写分离的实现与主从延迟解决方案（Java 后端视角）
> 在 Spring Boot 项目中，使用 ShardingSphere-JDBC 或动态数据源实现读写分离。请回答：
> 1. 读写分离的基本原理是什么？写操作走主库，读操作走从库，底层如何路由？
> 2. 主从延迟导致“写后读”不一致，有哪些解决方案？请至少说出三种，并说明其适用场景。
> 3. 如何在 Java 代码中强制某次查询走主库？（以 ShardingSphere 为例，或自定义注解 + AOP）
     > 追问：主从同步模式有异步、半同步、全同步，分别有什么优缺点？生产环境如何选择？

<details>
<summary><strong>点击展开标准解析</strong></summary>

- 读写分离原理：基于主从复制，应用层通过中间件（如 ShardingSphere、MyCat）或动态数据源（AbstractRoutingDataSource）解析 SQL，判断是读还是写，路由到主库或从库。通常写主库，读从库，主库通过 binlog 同步到从库。

- 主从延迟解决方案：
  1. 强制走主库：对于写后立即读的业务（如订单创建后跳转详情），在代码中强制该查询走主库。适用场景：对一致性要求高的关键业务。
  2. 半同步复制：主库等待至少一个从库确认收到 binlog 后再提交，减少延迟。适用场景：可容忍少量性能损失，要求数据安全。
  3. 并行复制：MySQL 5.7+ 支持基于组提交的并行复制，从库多线程重放，降低延迟。适用场景：主库写入并发高。
  4. 缓存标记：写入后设置缓存标记，读请求先检查标记，若存在则走主库，直到从库同步完成。适用场景：读多写少，可接受短暂不一致。

- 强制走主库：ShardingSphere 提供 HintManager 强制路由主库：
...
- 追问：异步复制性能最高但可能丢数据；半同步复制保证至少一个从库收到，性能略降；全同步复制（组复制）保证强一致但性能最差。生产环境通常选择半同步复制或异步复制 + 强制走主库策略。
</details>
